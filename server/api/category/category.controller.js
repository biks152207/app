...
function index(req, res) {
  return _category2.default.find().exec().then(respondWithResult(res)).catch(handleError(res));
}
...
function upsert(req, res) {
  ...
  return _category2.default.findOneAndUpdate({ _id: req.params.id }, req.body, { upsert: true, setDefaultsOnInsert: true, runValidators: true }).exec().then(respondWithResult(res)).catch(handleError(res));
}
